...
<h4><i class="glyphicon glyphicon-stats"></i> <?= Html::encode($this->title) ?></h4>
 ...
 <br>
  <h4 class="text-center"><?=$title?> <?="&nbsp;".Job::dateThai($start_date)." ".Yii::t('app','to')."  ".Job::dateThai($end_date);?>  </h4>
    <?=$this->render('_search_date_chart',['start_date'=>$start_date,'end_date'=>$end_date,'link'=>'chart-department'])?>
                  ...
                  foreach($model as $type){
                			if($type['total'] > 0){
                				echo"<tr>";
                					...
                					echo "<td class='text-center'>".Html::a(Yii::t('app','detail'),['print-detail','start'=>$start_date,'end'=>$end_date,'department_id'=>$type['id']],['class'=>'btn btn-default btn-xs','target'=>'_blank'])."</td>";
                				echo"</tr>";
                        $sum += $type['total'];
                				$i++;
                			}
